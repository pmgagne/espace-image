...
* **Compatibility:** Must work on both Modern (Blur, Flexbox/Grid) and Legacy (iPad 2, Absolute positioning, No Blur) slideshows.
...
* [x] **Dismissal**:
  ...
  * [x] **Implementation Note**: Added "Smart Swap" logic to both slideshows (Modern via `htmx:afterRequest`, Legacy via `lastAlarmHtml` tracker) to prevent repetitive `slideUp` animations when content hasn't changed.
