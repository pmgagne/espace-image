...
- **Type Hints:** ALWAYS use type hints for function signatures; Strict Pylance checking enabled
...
### Mocking & Integration
- Mock external services (Weather API, iCloud Calendar) in unit tests.
- Use fixtures for database sessions and temporary media directories.
- See `tests/conftest.py` for standard mocks and fixtures.
...
class ImageService:
    ...
    async def process_upload(self, data: bytes) -> str:
...
❌ Hardcode configuration or secrets (use environment variables or DB settings)
...
- Vendored libraries in `app/static/js/` (except `htmx.min.js` updates if explicitly asked)
- Generated files (`requirements.txt`, `uv.lock`)
- Database migrations (auto-generated by SQLModel)
- Build artifacts or databases in `data/` folder
